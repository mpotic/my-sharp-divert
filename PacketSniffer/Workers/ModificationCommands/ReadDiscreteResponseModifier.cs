...
{
	internal class ReadDiscreteResponseModifier : IPacketModifier
	{
		...
		public void RandomizeValues()
		{
			...
			byte[] values = new byte[payloadDataStr.Count(x => x.Equals(',')) + 1];
			values = values.Select(x => (byte)random.Next(0, 2)).ToArray();
			string newPayloadDataStr = ";;" + string.Join(',', values);
